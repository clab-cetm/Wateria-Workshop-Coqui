...
Coqui 透過 555 IC 發出震盪頻率，驅使蜂鳴器發出不同的音頻。安裝 555 的時候唯一要注意的就是 "安裝的方向"。在 555 的示意圖中有標示一個半圓形的缺口，並且有一個用來標記 "一號腳位" 的定位圓點。以下圖為例，圓點所在的腳位是編號一號的 GND (接地)。
...
圖片來源參考：[Build a Coquí: A Simple Water Conductivity Sensor (version 1), publiclab.org](https://publiclab.org/notes/ashkaya/09-17-2016/build-a-coqui-a-simple-water-conductivity-sensor#Step+by+Step+Instructions+for+Making+the+Coqui)
...
Coqui 所發出的音頻顯示了水質的導電度，在重金屬汙染的水質下，Coqui 會發出較高的音頻，就像跟人類抗議一樣。
...
事實上 555 Timer IC 的規格書都會附上不同的接法參考圖，以及該電路最終能產生頻率的計算公式。但其實最精準的方式就是直接量測訊號。


#### 使用示波器

我手上剛好有一台手持示波器，只要量測 Coqui 電路上蜂鳴器所連接的正負位置（紅黑線），你可以看到工作坊配置版本的 Coqui 將會輸出約 884Hz 的音頻，大約是 A5 的音符。
...
這個方式由來自巴塞隆納鄰里汙染空間監測所（[IMVEC.tech](https://imvec.tech/)）的 [Xose Quiroga](https://social.coop/@xosequiroga) 所提供。

事實上這個方式更簡單、便宜、而且有趣多了。直接使用吉他的調音器，甚至智慧型手機也有類似的 App 可以做到調音的效果。
...
需要注意，理論上當電阻值和電容值固定了，就能讓 Coqui 產生一個理想中的音階。但如果換了不同的電阻類型和電容類型，即使他們所標示的數值都一樣，最終所產生的音頻也可能會有所偏差。


請大家盡情享受改裝 Coqui 的樂趣吧！呱！呱！
